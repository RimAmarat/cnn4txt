...
class OnTheFlyRandomIterator(data.Iterator):
...
# learning
parser.add_argument('-lr', type=float, default=0.000007, help='initial learning rate [default: 0.001]')
...
def mr(text_field, label_field, **kargs):
    ...
    train_iter = CustomIterator(train_data, batch_size=32)
    dev_iter = CustomIterator(dev_data, batch_size=32)
...
tokens_french = tokenizer.tokenize(tokenizer.encode(train_data.data))
tokens_english = tokenizer.tokenize(tokenizer.encode(train_data.data))

print("tokens loaded")

# Get the embeddings (hidden states) for the tokens
with torch.no_grad():
    outputs = tokenizer(tokens_french + tokens_english, return_tensors="pt")

def preprocessor(batch):
    return outputs.encode(batch, add_special_tokens=True)

# load data
print("\nLoading data...")
# Define your fields
TEXT = data.Field(use_vocab=False,batch_first=True,sequential=True, lower=True, preprocessing=preprocessor)
...
args.embed_num = len(tokens_english) + len(tokens_french)
...
# Create iterator
test_iter = CustomIterator(test_data, batch_size=32)
